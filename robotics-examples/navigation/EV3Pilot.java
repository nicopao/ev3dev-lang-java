// had to change from Regulated motor to BaseRegulatedMotor
// the problem is that all synch operations were implemented within BaseRegulatedMotor
// need to put that into a sort of Synch Manager -- ignore for now
 ...
 * </p>
 **/
public class EV3Pilot implements  LineFollowingMoveController {
	
	private class SynchWrapper{
		private int synchStackSize;
		private EV3Pilot owner;
		
		public SynchWrapper(EV3Pilot owner) {
			this.owner=owner;
			this.synchStackSize=0;
		}
		
		public void startSynchronization() {
			this.synchStackSize=this.synchStackSize+1;
			if(this.synchStackSize==1)
				this.owner._left.startSynchronization();
		}
		public void endSynchronization() {
			if(this.synchStackSize==1)
				this.owner._left.endSynchronization();
			this.synchStackSize=this.synchStackSize-1;
		}
	}
	 ...
	 */
	public EV3Pilot(final double wheelDiameter,
			final double trackWidth, final BaseRegulatedMotor leftMotor,
			final BaseRegulatedMotor rightMotor) {
	...
	public EV3Pilot(final double wheelDiameter,
			final double trackWidth, final BaseRegulatedMotor leftMotor,
			final BaseRegulatedMotor rightMotor, final boolean reverse) {
	 ...
	 *            running backward.
	 */
	public EV3Pilot(final double leftWheelDiameter,
			final double rightWheelDiameter, final double trackWidth,
			final BaseRegulatedMotor leftMotor, final BaseRegulatedMotor rightMotor,
		...
		_left.synchronizeWith(new BaseRegulatedMotor[]{_right});
		
		this.synchwrap = new SynchWrapper(this);
