...
public class LCDStretch extends EV3DevDevice implements BrickLCD {
	...
	public void drawString(String str, int x, int y, boolean inverted) {
		...
		int adjusted_x = x * metrics.charWidth('_');
		int line_height = metrics.getHeight();
		int string_width = metrics.stringWidth(str);
		...
		this.drawString(str, adjusted_x, adjusted_y, anchor, inverted);
		this.refresh();
	}
	...
	@Deprecated
	@Override
	public void drawRegionRop(Image src, int sx, int sy, int w, int h, int transform, int x, int y, int anchor,
			int rop) {
		x = adjustX(x, w, anchor);
		y = adjustY(y, h, anchor);
	 ...
	 */
	@Override
	public void bitBlt(byte[] src, int sw, int sh, int sx, int sy, byte[] dst, int dw, int dh, int dx, int dy, int w,
			int h, int rop) {
		BufferedImage srcI = lejos2rgb(src, sw, sh);
